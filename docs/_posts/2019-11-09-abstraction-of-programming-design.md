...
思考一下，ATM 外在的行為是什麼？我們可以提款(withdraw)、存款(deposit)、查詢餘額(check balances)、查詢紀錄(query record)，所以讓我們先寫下三個定義，放進 `app.rkt` 中
...
現在讓我們可以回到 `check-balances` 了，我們一共採用了 3 個前面沒提到的東西 (真的要說其實只有兩個，這個等下就會知道)
...
當然，這個語法之所以存在就是因為這個變換並不好寫的關係，所以不要沒事改成 lambda 的寫法

`cond` 是 Scheme 的條件式之一，它接收一些 clause，回傳第一個成功的 clause 的結果，每個 clause 都由 ('predicate ‘expression…) 組成，注意在 Scheme 裡，多個表達式存在於一個地方的時候回傳值是最後一個表達式的值，換句話說如果中間表達式不具副作用那就是沒有意義的程式：
...
現在我們有一組 ATM 的核心功能，但我們在可預見的未來內就會對它們進行修改，也因此我們必須要開始編寫簡單的測試來保證程式的關鍵功能沒有被破壞
...
在 `atm.rkt` 中必須加入下面的程式來 export 這些函數
...
這是為什麼呢？因為我們的核心函式總是修改同一個全域的紀錄，這使得每次的測試之間其實都不是獨立的；然而當我們閱讀測試的時候，必然不希望每次都需要閱讀大量的前後文才能正確的理解結果 (事實上如果真實世界的測試寫成這樣也不可能讀得完)；因此我們必須要修改核心的函式
...
我們藉由引入一個變數去除了共用全域變數的衝突，注意 `app.rkt` 也要一起修改，這裡就不多費口舌
